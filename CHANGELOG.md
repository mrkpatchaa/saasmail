...
- **Native in-app mail agent UI.** A global Agent side panel now opens from the top navigation or Mod+J, manages caller-owned sessions, renders sanitized Markdown with tool-state badges, and opens saved drafts back into the existing compose/reply flows. Inbox admins can add up to 4000 characters of per-inbox agent instructions. Markdown images are deliberately not rendered so untrusted model/mail content cannot trigger automatic remote image or tracking-pixel fetches. When no model provider is configured, the panel remains available for session management and shows a configuration hint instead of adding a production mock.

- **Native in-app mail agent backend.** A per-user/per-session `MailAgent` Durable Object now stores chat transcripts separately from mail while all mail access continues through permission-scoped D1 services. Provider selection is Anthropic → OpenAI → Workers AI with exact-pinned AI/Agents dependencies and a configurable `AGENT_MODEL`. The Stage 2 tool surface can read/search mail, inspect customer history/templates/playbooks, organize message state, and save caller-owned drafts, but it cannot send, trash, delete, or enroll contacts. Agent sessions are caller-only, request auth is revalidated at the Durable Object boundary, session ownership is re-read from D1 each turn, and user role/inbox scope is refreshed on every tool call. Mail content is framed as quoted untrusted data rather than instructions, and reply drafting preserves any non-empty human autosave.
